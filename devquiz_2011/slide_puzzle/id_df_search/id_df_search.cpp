...
// プロトタイプ宣言
bool iddfs(string& result, STATE state, STATE goal, stack<STATE> iddfs_stack, HASH<string, unsigned int> hs, char** adjacent);

/* 探索 */
string id_df_search(int width, int height, string board)
{
    int size = width*height;        // パネル数
    char** adjacent;                // 隣接配列
    STATE start = new struct State; // スタートの局面状態
    STATE goal = new struct State;  // ゴールの局面状態
    stack<STATE> iddfs_stack;       // スタック
    ...
    for(int i = 1; i < 30; i++) {
        ...
        // 深さ優先探索
        iddfs(result, start, goal, iddfs_stack, hs, adjacent);
        ...
        if(buff.size() > 0) {
            ...
            for(it = buff.begin(); it != buff.end(); it++) {
                ...
                */
                ...
                int candidate_depth = (*candidate_state).depth + 1;
                int candidate_space;
                ...
                // 下限値枝刈り法
                if(candidate_cost + candidate_depth <= depth_limit) {
                    c = new struct State;
                    ...
                    // 再帰
                    if(iddfs(result, c, goal, iddfs_stack, hs, adjacent)) {
                        delete c;
