...
/* 探索 */
string a_star_search(int width, int height, string board)
{
    ...
    while(!astar_queue.empty()) {
        ...
        for(int i = 0; (n = adjacent[a->space][i]) != -1; i++) {
            ...
            if(a->block[n] != '=') {
                ...
                // 動かしたパネルの差分だけ計算
                diff_MD = manhattan_distance(panel, a->space+1) - manhattan_distance(panel, n+1);
