...
public:
    ...
    // 新規にキーと値のセットを追加します
    Iterator addPair(const TValue& val) {
        // Vectorの要素位置を取得
        const unsigned int hval = m_func(val->board);

        // セットの追加
        m_vec[hval].push_front(Pair(val->board, val));
